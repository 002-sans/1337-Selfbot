        ...
        switch(interaction.options.getSubcommand())
        {
            ...
            case 'list':
                ...
                const embed = {
                    ...
                    description: keys.map(key => `\`${key}\` - Expire <t:${Math.round(isNaN(codes[key].expiresAt) ? client.ms(codes[key].expiresAt) : codes[key].expiresAt / 1000)}:R> ${codes[key].used ? `- <@${codes[key].by}>` : ''}`).join('\n'),
                    color: 0x000000,
                }
